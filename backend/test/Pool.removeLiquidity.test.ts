...
// a cette suite : en general, Pool.sol:91 autorise une reserve strictement
...
// 10% du totalSupply du pool amorce (3 * SEED_AMOUNT = 30 000 000 000).
// Choisi parce qu'il divise proprement : chaque amountsOut[i] vaut alors
// exactement SEED_AMOUNT / 10 = 1 000 000 000, sans troncature qui
// polluerait les assertions du cas nominal (section II.A).
const BURN_AMOUNT = (3n * SEED_AMOUNT) / 10n;
...
//
// Sur ce pool precis (fraichement amorce, equilibre 1:1:1), l'aller-retour se
// revele en pratique exactement sans perte : totalSupply vaut alors
// exactement 3 x reserves[ancre], ce qui annule la troncature entiere dans
// les deux sens (au depot comme au retrait), quel que soit le montant
// depose. L'assertion ci-dessous reste un <= et non un == : c'est la
// propriete generale qu'on veut garantir (ne jamais creer de valeur), et
// l'egalite observee ici est un accident de cette configuration precise :
// un pool deja desequilibre, ou porteur de frais accumules, ne la
// presenterait pas necessairement.
async function assertRoundTripNeverExceedsDeposit(tokenIndex: 0 | 1 | 2) {
  ...
  const depositAmount = SEED_AMOUNT / 4n; // pool amorce balance : depot ancre sur token0 tire depositAmount sur chaque token

  await mintAndApprove(tokens, pool, other, depositAmount);
  ...
  describe("I] removeLiquidity sur pool vierge", function () {
    describe("A) Reverts", function () {
      it("totalSupply() vaut 0 : la division declenche un panic 0x12 (division par zero)", async function () {
        ...
        // mintee vers l'adresse morte des le premier depot (Pool.sol:79) et
  ...
  describe("II] removeLiquidity sur pool amorce", function () {
    describe("A) Cas nominal", function () {
      it("le retirant recoit 10% de chaque reserve sur les trois tokens", async function () {
        ...
        assert.deepEqual(
          ...
          `recu=[${received}], attendu 3x ${SEED_AMOUNT / 10n} (10% de chaque reserve)`,
        );
      });
      ...
      it("chaque reserve diminue exactement de ce que le retirant a recu", async function () {
        ...
        // formule interne (Pool.sol:107) en TS.
        const { pool, tokens, depositor } = await networkHelpers.loadFixture(deploySeededPoolFixture);
      ...
      it("la valeur de retour amountsOut vaut les trois montants transferes", async function () {
        ...
        // (3e9, 10% de supply) => amountsOut[i] = 1e10 * 3e9 / 3e10 = 1e9
        // pour les trois.
        const { pool, depositor } = await networkHelpers.loadFixture(deploySeededPoolFixture);
    ...
    describe("B) Reverts", function () {
      ...
      it("_minOut[0] strictement superieur au montant sortant : BadSlippage", async function () {
        const { pool, depositor } = await networkHelpers.loadFixture(deploySeededPoolFixture);
        const tooHighMinOut = SEED_AMOUNT / 10n + 1n;
      ...
      it("_minOut[1] strictement superieur au montant sortant : BadSlippage", async function () {
        // Le require vit dans la boucle (Pool.sol:106-110) : l'indice 1
      ...
      it("retrait trop grand ET _minOut trop exigeant : BadSlippage avant ERC20InsufficientBalance", async function () {
        // La boucle de slippage (Pool.sol:106-110) s'execute entierement
        // avant le _burn (Pool.sol:111) : un appel qui viole les deux gardes
      ...
      it("_burnedShares superieur au totalSupply : panic 0x11, le decrement des reserves sous-flow avant le _burn", async function () {
        // La boucle de decrement des reserves (Pool.sol:106-110) s'execute
        // avant le _burn (Pool.sol:111). Si _burnedShares depasse
    ...
    describe("C) Cas limites", function () {
      ...
      describe("2) Arrondi entier, toujours en faveur du pool", function () {
        it("bruler 1 seule part ne rend aucun token", async function () {
          ...
          // Calcul a la main : reserves[i] * _burnedShares / supply
          //                   = 1e10 * 1 / 3e10 = 0 (0.333... tronque a 0)
          const { pool, tokens, depositor } = await networkHelpers.loadFixture(deploySeededPoolFixture);
          const balancesBefore = await readBalances(tokens, depositor.account.address);
      ...
      describe("3) Retrait total et propriete des parts", function () {
        it("retirer la totalite de ses parts laisse un residu non nul dans chaque reserve", async function () {
          ...
          //          = 3 * SEED_AMOUNT - 1000 = 29 999 999 000
          //   amountsOut[i] = reserves[i] * burned / totalSupply
          //                 = 1e10 * 29 999 999 000 / 3e10
          //                 = 9 999 999 666 (tronque vers le bas)
          //   residu[i] = reserves[i] - amountsOut[i]
          //             = 1e10 - 9 999 999 666 = 334
          const { pool, depositor } = await networkHelpers.loadFixture(deploySeededPoolFixture);
          ...
          assert.deepEqual(
            reservesAfter,
            [334n, 334n, 334n],
            `reserves apres retrait total du solde libre=[${reservesAfter}], attendu [334n, 334n, 334n] (calcul a la main en commentaire)`,
          );
        });
      ...
      describe("4) Retraits successifs", function () {
        it("deux retraits successifs de la meme quantite de parts rendent exactement les memes montants", async function () {
          ...
          // Calcul a la main : reserves = [1e10, 1e10, 1e10], supply = 3e10.
          //   1er retrait : amountsOut[i] = 1e10 * 3e9 / 3e10 = 1e9
          //   apres : reserves = [9e9, 9e9, 9e9], supply = 2,7e10
          //   2e retrait : amountsOut[i] = 9e9 * 3e9 / 2,7e10 = 1e9 (identique)
          const { pool, tokens, depositor } = await networkHelpers.loadFixture(deploySeededPoolFixture);
    ...
    describe("D) Pool desequilibre", function () {
      describe("1) Composition et proportionnalite", function () {
        ...
        it("un retrait ne modifie pas la composition du pool", async function () {
          ...
          // sans reimplementer Pool.sol:107.
      ...
      describe("2) Consequence chiffree du desequilibre (calcul a la main)", function () {
        it("bruler 7 000 000 000 parts rend trois montants tronques vers le bas", async function () {
          // reserves = [1250e8, 1000e8, 800e8], supply = 300 000 000 000
          // amountsOut[i] = reserves[i] * 7 000 000 000 / 300 000 000 000
          //   token0 : 125 000 000 000 * 7 000 000 000 / 300 000 000 000 = 2 916 666 666,67 -> 2 916 666 666
          //   token1 : 100 000 000 000 * 7 000 000 000 / 300 000 000 000 = 2 333 333 333,33 -> 2 333 333 333
          //   token2 :  80 000 000 000 * 7 000 000 000 / 300 000 000 000 = 1 866 666 666,67 -> 1 866 666 666
          const { pool, tokens, depositor } = await networkHelpers.loadFixture(deployImbalancedPoolFixture);
          const balancesBefore = await readBalances(tokens, depositor.account.address);
          const burnedShares = 7_000_000_000n;
          ...
          assert.deepEqual(
            received,
            [2_916_666_666n, 2_333_333_333n, 1_866_666_666n],
            `recu=[${received}], attendu=[2916666666n, 2333333333n, 1866666666n] (calcul a la main en commentaire)`,
          );
        });
      });
      ...
      describe("3) Evenement avec des montants distincts", function () {
        it("RemovedLiquidity porte trois amountsOut distincts sur un pool desequilibre", async function () {
          ...
          await viem.assertions.emitWithArgs(
            ...
            pool,
            "RemovedLiquidity",
            [depositor.account.address, [2_916_666_666n, 2_333_333_333n, 1_866_666_666n], burnedShares],
          );
        });
      });
    });
